...
<c:if test="${kinds eq '2'}">
	...
	<script>
		...
		function goPage(page,_this) {
			...
			$("#form").attr("action", "/proBoard/list");
			$("#form").submit();
		}
	...
	<section class="banner-area logo3 relative" id="home">	
			...
			<div class="container wow bounceInUp" data-wow-duration="1.4s">
				...
				<div class="row portfolio-container2">
			  	...
			  	<c:if test="${pageMaker.prev}">
				    <li class="page-item">
				      <a class="page-link" href='<c:url value="/proBoard/list?page=${pageMaker.startPage-1}"/>' aria-label="Previous">
			    ...
			    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
					<li class="page-item ${pageMaker.cri.page eq idx?" active":""}">
						<a class="page-link waves-effect waves-effect" href='<c:url value="/proBoard/list?page=${idx }"/>'>${idx}</a>
					</li>
				</c:forEach>
				
			    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
				    <li class="page-item">
				      <a class="page-link" href='<c:url value="/proBoard/list?page=${pageMaker.endPage+1}"/>' aria-label="Next">
